...
// iOS 14+ 上 __DATA_CONST 是只读保护段，写前必须 vm_protect 解锁
static void safe_write_ptr(void **slot, void *value) {
    if (*slot == value) return;
    mach_port_t task = mach_task_self();
    uintptr_t addr  = (uintptr_t)slot;
    uintptr_t page  = addr & ~((uintptr_t)(vm_page_size - 1));
    vm_protect(task, page, vm_page_size, false, VM_PROT_READ | VM_PROT_WRITE);
    *slot = value;
